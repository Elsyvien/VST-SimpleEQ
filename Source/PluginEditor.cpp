    ...
    float rotaryStartAngle, float rotaryEndAngle, juce::Slider& slider)
{
    ...
    g.setColour(Colour(52u, 172u, 188u));
    g.fillEllipse(bounds);

    g.setColour(Colour(133u, 195u, 207u));
        ...
        bool shouldDrawButtonAsDown);
{
    ...
    else if(auto* analyzerButton =  dynamic_cast<AnalyzerButton*>(&toggleButton))
    {
        ...
        {
            randomPath.lineTo(x, insectRect.getY() + insectRect.getHeight() * r.nextFloat());
        }
...
void RotarySliderWithLabels::paint(juce::Graphics &g)
{
    ...
    g.drawRect(sliderBounds);*/ //only meant for debbuging purposes not necessery anymore
...
void ResponseCurveComponent::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < w; ++i) {
        double mag = 1.f;
        auto freq = mapToLog10(double(i) / double(w), 20.0, 200000.0);
...
{
    auto bounds = getLocalBounds();
    float hRatio = 25.f / 100.f; //JUCE_LIVE_CONSTANT(33) / 100.f; //Meant for live Adjusting size
